...
const PROVIDERS = [
  ...
  {
    id: 'MONEY_MANAGER_SQLITE',
    name: 'Money Manager (SQLite)',
    description: 'Importar base de datos de respaldo (.sqlite, .db) desde tu dispositivo',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
...
export default function ImportWidget({ onAnalyzed, onError }: ImportWidgetProps) {
  ...
  const processFile = async (file: File) => {
    ...
    if (selectedProvider === 'MONEY_MANAGER_SQLITE') {
      if (extension !== 'sqlite' && extension !== 'db') {
        onError('Por favor, selecciona un archivo SQLite válido (.sqlite, .db)');
        return;
      }
    } else {
    ...
    <div className="w-full max-w-2xl mx-auto space-y-6">
      ...
      <div className="space-y-2">
        ...
        <input
          ...
          accept={selectedProvider === 'MONEY_MANAGER_SQLITE' ? '.sqlite,.db' : '.xls,.xlsx,.csv'}
          className="hidden"
        />
            ...
            <div className="flex flex-col items-center text-center p-6 space-y-4">
              ...
              <div>
                ...
                <p className="text-slate-400 text-xs mt-1">
                  o haz clic para buscar en tu dispositivo ({selectedProvider === 'MONEY_MANAGER_SQLITE' ? '.sqlite, .db' : '.xls, .xlsx, .csv'})
                </p>
              </div>
